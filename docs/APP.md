...
A block after `THEN` can set several channels. `ELSE IF` works. Compares are 0/1, so `(ch1>0)+(ch2>0)>1` means two high. Logic: `AND` `OR` `NOT` (`&&` `||` `!`). Math: `+ - * / % abs() min() max() sqrt() pow(a, b)`. `LET name = expr` or `LET name, expr`. `FOR <seconds>` keeps the body on for that many seconds even if the outer `IF` later misses. `UNTIL <cond>` keeps the body on until the cond is true (`UNTIL ch4 < ch2`). Names: `ch`/`last`/`mean`/`rms`/`prev`/`dxmean`/`above`/`pos`/`signal`, and `ch1`…`ch8`.
