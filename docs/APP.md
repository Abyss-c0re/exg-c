...
- **USB / LAN / Bluetooth** next to Connect. USB is the Knight here. Bluetooth picks a nearby name (Allow/No) and EXG rides that link. Share EXG asks to be findable so the scan can see it. LAN is EXG on wifi after a pair that saw wifi. Share EXG is how this device offers the board out.
