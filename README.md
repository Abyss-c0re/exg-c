...
Shipped app: **2.70** (`com.abysscore.exgc`, versionCode 78). One framework, two skins:
...
- **Cube viz** — one 2×2×2 cube is 8 bits, one cell per channel. Pick the algo on the Cube tab. **custom** is per-bit if/else (`ch1`…`ch8` last µV; tap a bit to edit, long-press to pick its channel). **float on/off**, drag to spin, +/− or pinch to zoom. Sites live in Settings.
